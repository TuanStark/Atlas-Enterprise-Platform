...
export class PageRequest implements Pagination {
  ...
  constructor(
    ...
    const validatedMaxLimit =
      Number.isInteger(maxLimit) && maxLimit > 0 ? maxLimit : PageRequest.DEFAULT_MAX_LIMIT;
...
export class CursorPageRequest implements CursorPagination {
  ...
  constructor(
    ...
    const validatedLimit =
      Number.isInteger(limit) && limit > 0 ? limit : CursorPageRequest.DEFAULT_LIMIT;
    const validatedMaxLimit =
      Number.isInteger(maxLimit) && maxLimit > 0 ? maxLimit : CursorPageRequest.DEFAULT_MAX_LIMIT;
